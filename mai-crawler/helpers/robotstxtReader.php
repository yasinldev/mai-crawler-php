...
class readfile{
    ...
    public int $empty_lines = 0;
    public string $useragent = "";
    public int $crawl_delay = 0;

    public function read_robots($comments, $emptyLines){
        $fl = fopen(ROOT . "/json/robots/robots.txt", "r") or die("Unable to open file!");
        $ofile = file( ROOT . "/json/robots/robots.txt") or die("Unable to open file!");
        $count = 0;
        ...
        // JSON encode rules
        fopen(ROOT . "/json/robots/robots.json", "w+") or die("Unable to open file!");
        $json = json_encode($this, JSON_PRETTY_PRINT);
        if(file_put_contents(ROOT . "/json/robots/robots.json", $json)){
            return true;
        } else{
